    ...
    go.GraphObject.defineBuilder('ScrollingTable', (args) => {
      ...
      function setScrollIndexLocal(bar: go.GraphObject | null, y: number) {
        ...
        const idx = Math.round(
          Math.max(0, Math.min(1, (y - uph) / (tabh - uph - downh))) *
            table.rowCount
        );
      ...
      return $(
        ...
        $(
          ...
          'Table',
          {
            ...
            background: '#DDDDDD',
            mouseEnter: (e, bar: go.GraphObject) =>
              showScrollButtons(bar as go.Panel, true),
            mouseLeave: (e, bar: go.GraphObject) =>
              showScrollButtons(bar as go.Panel, false),
          },
